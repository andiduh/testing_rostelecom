...
elements_for_test_tab_displayed = (
    ...
    ("p", "card-container__desc", "Укажите почту или номер телефона, на которые необходимо отправить код подтверждения"),
...
def test_authorization_with_code_and_change_phone(driver_module, page_waiter_module):
    ...
    # Проверка перенаправления на страницу с кодом подтверждения
    cur_URL = driver_module.current_url
    assert re.match(r"https://b2c.passport.rt.ru/auth/realms/b2c/login-actions/authenticate?execution.+",
                    cur_URL)
    print('Открылась страница для ввода кода подтверждения.')
...
#   time.sleep(15) # Сделать строку активной для заполнения капчи вручную
    ...
    login_button.click()
    time.sleep(5)
    code_input = driver_module.find_element(By.ID, value="rt-code-input")  # Ввод номера телефона в поле
    code_input.click()
