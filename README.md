...
## Внимание! Возможные падения тестов:
- При активных действиях на сайте (авторизация/регистрация и тд) система выдает капчу на вход, что блокирует работу автотеста и результат будет FAILED.
- Из-за бага №6 "Нет возможности работать с системой при получении Кода подтверждения, пока не истекло время таймера для предыдущего кода (120 секунд)" некоторые автотесты падают, так как система зависает на 120 секунд. И также результат будет FAILED.
  Рекомендую тесты с FAILED запустить потом отдельно.
